...
void solveVolumeConstraints() {
	...
	parallel_for((size_t)0, _volumeConstraintData.constraintCount - 1, (size_t)1, [=](size_t i) {
		// get actual vertices
		vec3 verts[4];
		verts[0] = _tetMeshVertices[get<0>(_volumeConstraintData.vertexIds[i])];
		verts[1] = _tetMeshVertices[get<1>(_volumeConstraintData.vertexIds[i])];
		verts[2] = _tetMeshVertices[get<2>(_volumeConstraintData.vertexIds[i])]; 
		verts[3] = _tetMeshVertices[get<3>(_volumeConstraintData.vertexIds[i])];
...
void solveDistanceConstraints() {
	...
	parallel_for((size_t)0, _distanceConstraintData.constraintCount - 1, (size_t)1, [=](size_t i) {
		...
		vec3 delta = ((_tetMeshVertices[id1] - _tetMeshVertices[id2]) / currentDistance) * (_distanceConstraintData.restValues[i] - currentDistance) / 2.0f;
...
#pragma region Getters
	...
	DLL_EXPORT void dll_getVertices(int* vertexOutput) {
		...
		memcpy(vertexOutput, result.data(), _tetMeshVertices.size() * 3 * sizeof(float));
	...
	DLL_EXPORT void dll_getTetrahedra(int* tetOutput) {
		...
		memcpy(tetOutput, result.data(), _tetMeshTetrahedra.size() * 4 * sizeof(int));
	...
	DLL_EXPORT void dll_getTetMeshSurfaceVertices(int* output) {
		...
		memcpy(output, result.data(), newSurfVerts.size() * 3 * sizeof(float));
	...
	DLL_EXPORT void dll_getTetMeshSurfaceTriangles(int* output) {
		...
		memcpy(output, result.data(), _tetMeshSurfaceTriangles.size() *3* sizeof(int));
		vector<int>().swap(result);
	}
