...
vector<int> _initExecutionTimes;	// total, read tetwild file, constraint gen., barycentric mapping, serial save, serial load 
vector<int> _solverExecutionTimes;	// total, project, solver iterations 1, ..., solver iterations 10, bc update

DistanceConstraintData _distanceConstraints;
VolumeConstraintData _volumeConstraints;
...
void teardown() {
	...
	_distanceConstraints = DistanceConstraintData();
	_volumeConstraints = VolumeConstraintData();
...
// projects vertices onto the surface of a collider
void projectVertices_s(const vec3& collPos, const vec3& collSize, const int collType) {
	_collidingVertexCount = 0;
	for( int i=0; i< _tetMeshVertices.size(); i++){
		vec3 vertex = geometry::rotate(_tetMeshVertices.data()[i], _tetMeshRotation) + _tetMeshPosition;
		if (doesCollide(vertex, collPos, collSize, collType)) {
			_collidingVertexCount++;
			vertex = projectOrthogonal(vertex, collPos, collSize, collType);
...
void solveVolumeConstraintsGaussSeidel() {
	for (int i = 0; i < _volumeConstraints.constraintCount; i++) {
		...
		// determine delta
		float currentVolume = geometry::getTetrahedronVolume(verts[0], verts[1], verts[2], verts[3]);
		_volumeConstraints.restValues[i] = misc::lerp(_volumeConstraints.restValues[i], currentVolume, _plasticityFactor);
		float volumeDifference = currentVolume -_volumeConstraints.restValues[i];
...
void solveVolumeConstraintsJacobi() {
	...
	// apply deltas
	parallel_for((size_t)0, (size_t)(_tetMeshVertices.size() - 1), (size_t)1, [=](size_t i) {
		_tetMeshVertices[i] += _volumeDeltas[i] / (float)_volumeConstraints.constraintCountPerVertex[i];
	});
	// update rest values
	parallel_for((size_t)0, _volumeConstraints.constraintCount - 1, (size_t)1, [=](size_t i) {
		vec3 verts[4];
...
void solve_s() {
	...
	for (int collisionId = 0; collisionId < _collisions.size(); collisionId++) {
		int collisideId = _collisions[collisionId];
		projectVertices_s(_collData.colliderPositions[collisideId], _collData.colliderSizes[collisideId], _collData.colliderTypes[collisideId]);
	}
	_collisions.resize(0);
	chrono::duration<float> duration = chrono::high_resolution_clock::now() - startTime;
	_solverExecutionTimes[1] = chrono::duration_cast<chrono::milliseconds>(duration).count();
...
void setVolumeStiffness(float s) {
	_volume_stiffness = clamp(s, 0.f, 1.f);
	logger::log("--setVolumeStiffness to " + to_string(s));
}

void setDistanceStiffness(float s) {
	_distance_stiffness = clamp(s, 0.f, 1.f);
	logger::log("--setDistanceStiffness to " + to_string(s));
}
...
bool initFromFile(string fileName) {
	logger::log("--init from file "+_tetrahedralizationPath + fileName);
	_initExecutionTimes.resize(7, 0);
	_solverExecutionTimes.resize(4, 0);

	auto initStartTime = chrono::high_resolution_clock::now();
	Serialization::load(
		_tetrahedralizationPath+fileName,
		_tetMeshVertices,
		_tetMeshTetrahedra,
		_surfaceVertices,
	...
	_debugInt = _barycentricTetIds.size();
	_debugFloat = _tetMeshVertices.size();

	_initExecutionTimes[6] = chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - initStartTime).count();
...
bool init_m() {
	...
	auto bcStartTime1 = chrono::high_resolution_clock::now();
	logger::log("Barycentric Mapping...");
	bcmapping::findBarycentricTetIds_m(
		_surfaceVertices,
		_tetMeshVertices,
		_tetMeshTetrahedra,
		_surfaceVertexToTetVertexMap,
	...
	_initExecutionTimes[3] = chrono::duration_cast<chrono::milliseconds>(bcStopTime1 - bcStartTime1).count();
	auto bcStartTime2 = chrono::high_resolution_clock::now();
	bcmapping::generateBarycentricMapping_m(
		_surfaceVertices,
		_tetMeshVertices,
		_tetMeshTetrahedra,
		_surfaceVertexToTetVertexMap,
...
void storeData() {
	auto serializationStartTime = chrono::high_resolution_clock::now();
	logger::log("Serialization...");
	Serialization::save(
		_tetrahedralizationPath + _storeFileName + ".tetMesh",
		_tetMeshVertices,
		_tetMeshTetrahedra,
		_surfaceVertices,
		_surfaceVertexToTetVertexMap,
		...
		_distanceConstraints,
		_volumeConstraints);
	auto serializationStopTime = chrono::high_resolution_clock::now();
	_initExecutionTimes[5] = chrono::duration_cast<chrono::milliseconds>(serializationStopTime - serializationStartTime).count();
}
#pragma endregion Setters

// EXPORT FUNCTIONS
// TODO_: change the size of the memcpy vector to result.size() * sizeof(x) (rather than _some_vector.size()*3*sizeof(x))
extern "C" {
#pragma region Setters
	...
	DLL_EXPORT void dll_setVolumeStiffness(float s) {
		setVolumeStiffness(s);
	}
	...
	DLL_EXPORT void dll_setFilePath(const char* path, int charCount) {
		...
		//_tetrahedralizationPath = _projectPath + "Tetrahedralization/Peformance/Tests/371";
		_logPath = _projectPath + "Logs/";
		logger::setFilePath(_logPath);
	}
	DLL_EXPORT void dll_setTetrahedralizationPath(const char* path, int charCount) {
		_tetrahedralizationPath = vectorFuncs::charPtrToString(path, charCount);
	}
	DLL_EXPORT void dll_setStoreFileName(const char* name, int charCount) {
		_storeFileName = vectorFuncs::charPtrToString(name, charCount);
	}
#pragma endregion Setters
#pragma region Getters
	...
	DLL_EXPORT void dll_getSurfaceVertices_m(int* output) {
	...
	DLL_EXPORT void dll_getSurfaceVertices_s(int* output) {
		// get vertex positions for barycentric mapping
		vector<vec3> newSurfVerts;

		auto startTime = chrono::high_resolution_clock::now();
		newSurfVerts.resize(_surfaceVertices.size());
		bcmapping::updateSurfaceVertices_s(
			newSurfVerts,
			_tetMeshVertices,
			_tetMeshTetrahedra,
			_barycentricCoordinates,
	...
	DLL_EXPORT void dll_getInitExecutionTimes(int* output) {
		memcpy(output, _initExecutionTimes.data(), (int)_initExecutionTimes.size() * sizeof(int));
	}
	DLL_EXPORT void dll_getSolverExecutionTimes(int* output) {
		memcpy(output, _solverExecutionTimes.data(), (int)_solverExecutionTimes.size() * sizeof(int));
	}
